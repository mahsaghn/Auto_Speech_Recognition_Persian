...
def run_evaluation(test_loader,
    ...
    for i, (batch) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        with open('output_data.txt', 'a') as outfile:
...
def model_out(test_loader,
    ...
    for i, (batch)  in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        with autocast(enabled=precision == 16):
            out, output_sizes = model(inputs, input_sizes)
        out_resh = out.permute(1,0,2)
        out_dic = {}
        #print(out_resh.size())
        out_dic['mo'] = out_resh
        out_dic['ro'] = targets
        out_dic['tl'] = target_sizes
        #879 start dev, 1306 start test
        filepath = os.path.join('../../../s2s', f'output-{1306+i}.pth')
        torch.save(out_dic,filepath)
